...
export default function UfficioViaggiSpeditiPage() {
  ...
  async function load() {
    ...
    const res = await supabase
      .from("pallets")
      .select(
        "id, client, pallet_no, bobbins_count, shipping_type, dimensions, trip_id, sent_at, created_at, trip:trip_id(trip_date)"
      )
      .not("sent_at", "is", null);
  ...
  const grouped = useMemo(() => {
    ...
    // ordina i gruppi: prima "senza viaggio", poi date desc (gli spediti più recenti in alto)
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm" style={{ color: "var(--muted)" }}>
              Storico solo visura + eliminazione definitiva. Raggruppati per Data viaggio → Cliente.
            </p>
          </div>
